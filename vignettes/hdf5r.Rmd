...
**h5** on CRAN and **rhdf5** on Bioconductor. 




These are also good implementations, but there are several points that
...
- Tracking and closing of unused HDF5 resources is done automatically using the R garbage collector.

If you want to follow along with the code without having to copy/paste, the R-code can be found using the command
```{r, eval=FALSE}
system.file("../doc/hdf5r.R", package="hdf5r")
```
...
and the documentation that is listed there as well as specifically the [reference manual](https://www.hdfgroup.org/HDF5/doc/RM/RM_H5Front.html).
